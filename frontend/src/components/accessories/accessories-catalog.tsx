...
import { ArrowRight, Puzzle, SlidersHorizontal } from "lucide-react";
...
const STORAGE_KEY = "accessories-filters";

export function AccessoriesCatalog({ products }: { products: Accessory[] }) {
  const t = useT();
  ...
  }, [brands, types, colors, restored]);

  const filtered = useMemo(
    () =>
      products.filter((p) => {
        if (brands.size > 0 && !brands.has(p.brand)) return false;
        if (types.size > 0 && !types.has(p.accessoryType)) return false;
        if (colors.size > 0 && !colors.has(p.color)) return false;
        return true;
      }),
    [products, brands, types, colors],
  );
  ...
  const toggleBrand = mk(setBrands);
  const toggleType = mk(setTypes);
  const toggleColor = mk(setColors);
                ...
                <div className="relative aspect-square overflow-hidden bg-muted">
                  {p.images[0] ? (
                    ...
                    <div className="flex size-full items-center justify-center bg-gradient-to-br from-brand/10 via-muted to-brand-accent/10">
                      <Puzzle className="size-12 text-brand/25" />
                    </div>
                  )}
                </div>
